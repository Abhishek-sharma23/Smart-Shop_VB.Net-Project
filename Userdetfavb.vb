Public Class Userdetfavb
    Private Sub Userdetfavb_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ListView1.Columns.Add("UID", 80, HorizontalAlignment.Left)
        ListView1.Columns.Add("User Name", 150, HorizontalAlignment.Left)
                ...
                Using cmd As New OleDb.OleDbCommand(query, conn)
                    Using reader As OleDb.OleDbDataReader = cmd.ExecuteReader()
                        If reader.HasRows Then
                            While reader.Read()
                                Dim item As New ListViewItem(reader("UID").ToString())
                                item.SubItems.Add(reader("UName").ToString())
                                item.SubItems.Add(reader("UPno").ToString())
